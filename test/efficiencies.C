...
void plot_efficiencies( TFile* file, Int_t type = 2, TDirectory* BinDir, TString fin)
{
   ...
   if (TString(BinDir->GetName()).Contains("multicut")){
      ...
      fname = "plots/" + fprepend + "_" + hNameRef + fin(4,3);
   }
   if (__SAVE_IMAGE__){
   	fname += fin(4,3);
...
void efficiencies( TString fin = "TMVA.root", Int_t type = 2, Bool_t useTMVAStyle = kTRUE )
{
   ...
   while ((key = (TKey*)nextDir())) {
      ...
      if (path.Contains("multicutMVA")){
         multiMVA=kTRUE;
         plot_efficiencies( file, type, d,fin);
      }
   }
   plot_efficiencies( file, type, gDirectory, fin );
