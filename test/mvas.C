...
void mvas( TString fin = "TMVA_500.root", HistType htype = MVAType, Bool_t useTMVAStyle = kTRUE )
{
   cout << "file " << fin << endl; 
   ...
   while ((key = (TKey*)next())) {
      ...
      while ((titkey = (TKey*)keyIt())) {
         ...
         if (Save_Images) {
            if      (htype == MVAType)     {
            TString fname = Form("plots/mva_%s",     methodTitle.Data() );
            fname += fin(4,3);
            TMVAGlob::imgconv( c, fname ); }
            else if (htype == ProbaType)   TMVAGlob::imgconv( c, Form("plots/proba_%s",   methodTitle.Data()) ); 
            else if (htype == CompareType) {
            TString fname = Form("plots/overtrain_%s",     methodTitle.Data() );
            fname += fin(4,3);
            TMVAGlob::imgconv( c, fname ); }
