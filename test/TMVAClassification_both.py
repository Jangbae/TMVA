...
DEFAULT_OUTFNAME = "TMVA"+sys.argv[2]+".root"
DEFAULT_INFNAME  = "180"
...
#    gROOT.LoadMacro   ( "./TMVAGui.C" )
    ...
    infname = "ChargedHiggs_HplusTB_HplusToTB_M-%s_13TeV_amcatnlo_pythia8_hadd.root" %(massPoint)
...
#BDT machinary
    ...
    for i in range(len(varsList.bkg)):
        bkg_list.append(TFile.Open(preFix+varsList.bkg[i]))
        print preFix+varsList.bkg[i]
    ...
    # ====== register trees ====================================================
    factory.SetSignalWeightExpression('TrigEffWeight*pileupWeight*isoSF*lepIdSF*EGammaGsfSF*MuTrkSF*(MCWeight_singleLepCalc/abs(MCWeight_singleLepCalc))*xsecEff')
    factory.SetBackgroundWeightExpression('TrigEffWeight*pileupWeight*isoSF*lepIdSF*EGammaGsfSF*MuTrkSF*(MCWeight_singleLepCalc/abs(MCWeight_singleLepCalc))*xsecEff')
...
#    mycutSig = TCut( "NJetsCSV_JetSubCalc >= 3 && NJetsCSVwithSF_JetSubCalc >= 3") 
    mycutSig = TCut( "NJetsCSV_JetSubCalc >= 3") 
#    mycutBkg = TCut( "NJetsCSV_JetSubCalc >= 3 && NJetsCSVwithSF_JetSubCalc >= 3") 
    mycutBkg = TCut( "NJetsCSV_JetSubCalc >= 3") 
...
#    if "Fisher" in mlist:
 ...
 #       factory.BookMethod( TMVA.Types.kFisher, "Fisher")
    ...
    bdtSetting += ':MinNodeSize=2.5%:MaxDepth=3:BoostType=AdaBoost:AdaBoostBeta=0.5:UseBaggedBoost:BaggedSampleFraction=0.5:SeparationType=GiniIndex:nCuts=50'
